...
if __name__ == '__main__':
    ...
    numOfKeyFrames = len([f for f in os.listdir("./KeyFrames/")
                          if os.path.isfile(os.path.join("./KeyFrames/", f))])
    print('Nums of Key Frames: ', numOfKeyFrames)
    counter = 0
    while counter < numOfKeyFrames:
        print("Processing for KeyFrame{0}".format(counter))
        # reading the input image
        imgg = cv2.imread("./KeyFrames/frame{0}.jpg".format(counter), 0)
